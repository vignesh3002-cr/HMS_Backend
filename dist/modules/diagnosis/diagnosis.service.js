...
class DiagnosisService {
    async validateReferences(data) {
        ...
        if (data.histomorphology_id) {
            const histomorphology = await repository.findHistomorphology(data.histomorphology_id);
            if (!histomorphology) {
                throw new Error("Histomorphology not found");
            }
        }
        if (data.histological_grade_id) {
            const grade = await repository.findHistologicalGrade(data.histological_grade_id);
            if (!grade) {
    ...
    async createDiagnosis(data, createdBy) {
        ...
        return prisma_1.default.$transaction(async (tx) => {
            const diagnosisId = await (0, idGenerator_1.generateId)(tx, "DIAGNOSIS");
    ...
    async updateDiagnosis(diagnosisId, data, updatedBy) {
    ...
    async deleteDiagnosis(diagnosisId, updatedBy) {
        const existing = await repository.findById(diagnosisId);
        if (!existing) {
            throw new Error("Diagnosis not found");
        }
    ...
    async restoreDiagnosis(diagnosisId, updatedBy) {
        const existing = await repository.findById(diagnosisId);
        if (!existing) {
            throw new Error("Diagnosis not found");
        }
