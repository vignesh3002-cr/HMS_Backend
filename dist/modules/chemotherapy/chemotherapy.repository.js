...
class ChemotherapyRepository {
    ...
    async getMedicinesByCancerTypesAndSubtypes(cancerTypeIds, subtypeIds, drugRole) {
        ...
        const buildProtocolConditions = (useSubtypes) => {
            const conditions = [
                {
                    OR: [
                        { active_status: 1 },
        ...
        if (normRole === "DISCHARGE" || normRole === "POSTMEDICATION") {
            const queryDischarge = async (conds) => {
                return prisma_1.default.chemotherapy_discharge_instructions.findMany({
                    where: {
                        OR: [{ active_status: 1 }, { active_status: null }],
        ...
        const queryProtocolItems = async (conds) => {
            return prisma_1.default.chemotherapy_regimen_protocol_items.findMany({
                where: {
                    drug_role: normRole,
                    OR: [{ active_status: 1 }, { active_status: null }],
                    chemotherapy_regimen_protocol: {
    ...
    // given cancer type/subtype and drug role.
    async getMedicinesByCancerTypeAndSubtype(cancerTypeId, subtypeId, drugRole) {
        return this.getMedicinesByCancerTypesAndSubtypes([cancerTypeId], subtypeId ? [subtypeId] : undefined, drugRole);
    }
    ...
    async getProtocolFieldOptions() {
        const [dosageUnits, dilutionForms, dilutionDoseUnits, dilutionVolumeUnits, diluents, treatmentIntents] = await Promise.all([
            ...
            prisma_1.default.chemotherapy_regimen_protocol.findMany({
                where: {
        ...
        const standardDosageUnits = ["mg", "mg/m2", "mg/kg", "AUC", "g", "mcg"];
        const standardForms = ["IV Infusion", "IV Piggyback", "IV Push", "IV Bag", "Oral", "SC", "Tab", "Cap"];
        const standardDoseUnits = ["mg", "g", "mcg", "mg/m2", "mg/kg"];
        const standardVolumeUnits = ["mL", "L"];
        const standardDiluents = [
        ...
        const mergeDistinct = (dbList, defaults) => {
            const set = new Set();
        ...
        return {
            dosage_units: mergeDistinct(dosageUnits.map((d) => d.dosage_unit), standardDosageUnits),
    ...
    async listTreatmentIntents() {
        const rows = await prisma_1.default.chemotherapy_regimen_protocol.findMany({
            where: {
                treatment_intent: { not: null },
                OR: [{ active_status: 1 }, { active_status: null }]
            },
