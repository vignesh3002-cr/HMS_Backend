...
class ChemotherapyRepository {
    ...
    async getMedicinesByCancerTypesAndSubtypes(cancerTypeIds, subtypeIds, drugRole) {
        ...
        if (normRole === "DISCHARGE" || normRole === "POSTMEDICATION") {
            const dischargeItems = await prisma_1.default.chemotherapy_discharge_instructions.findMany({
                where: {
                    active_status: 1,
        ...
        if (seen.size === 0) {
            const fallbackItems = await prisma_1.default.chemotherapy_regimen_protocol_items.findMany({
                where: {
                    drug_role: normRole,
                    active_status: 1,
                    medicine_master: { is_active: true }
