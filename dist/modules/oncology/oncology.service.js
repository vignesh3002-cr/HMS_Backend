...
class OncologyValidationError extends Error {
    violations;
...
exports.OncologyValidationError = OncologyValidationError;
// ---------------------------------------------------------------------------
...
function mapMolecularRowToInput(row) {
    if (!row) {
        return {};
    }
    return {
...
function computePatientAge(patient) {
...
function computeIcdCascade(cancerType, subtype) {
...
function derivedPersistPayload(derived) {
    return {
...
class OncologyService {
    repository = new oncology_repository_1.OncologyRepository();
    // ---------------------------------------------------------------
    // Reference lookups
    // ---------------------------------------------------------------
    ...
    async listCancerSubtypes(cancerTypeId) {
    ...
    async listStagingReference(cancerTypeId) {
        const cancerType = await this.repository.findCancerTypeById(cancerTypeId);
        if (!cancerType) {
            throw new Error("Cancer type not found");
        }
    ...
    async reseedReferenceData() {
        const seedModule = require("../../../prisma/seedOncology");
    ...
    // ---------------------------------------------------------------
    async resolveCancerTypeAndSubtype(cancerTypeId, cancerSubtypeId) {
        const cancerType = await this.repository.findCancerTypeById(cancerTypeId);
        if (!cancerType) {
            throw new Error("Cancer type not found");
        }
    ...
    async resolveQualifyingEncounter(patientId) {
        ...
        if (encounter.status === oncology_constants_1.ENCOUNTER_OPEN_STATUS) {
            return encounter;
        }
        const ageDays = (Date.now() - new Date(encounter.encounter_ts).getTime()) / (1000 * 60 * 60 * 24);
        if (ageDays > oncology_constants_1.ENCOUNTER_RECENCY_WINDOW_DAYS) {
            throw new Error(`Patient's most recent encounter (${encounter.encounter_no}) is closed and older than ${oncology_constants_1.ENCOUNTER_RECENCY_WINDOW_DAYS} days. Open a new encounter before recording an oncology diagnosis.`);
        }
        return encounter;
    }
    async createStagingDetail(dto, actingUserId) {
        ...
        const ihc = { ...(dto.ihc ?? {}) };
        const molecular = { ...(dto.molecular ?? {}) };
        const validation = (0, chemo_validation_1.validateOncologyRecord)(staging, ihc, molecular);
        ...
        const derived = (0, chemo_derivation_1.deriveOncologyFields)(staging, ihc, molecular, { patientAgeYears, icd10FromSubtype: cascade.icd10_code, icdO3FromSubtype: cascade.icdO3Combined }, params);
        const stagingDetailId = await prisma_1.default.$transaction(async (tx) => {
            ...
            await this.repository.createStagingDetail(tx, {
                ...
                n_stage: dto.n_stage ?? null,
                m_stage: dto.m_stage ?? null,
                metastasis_sites: jsonOrUndefined(dto.metastasis_sites) ?? client_1.Prisma.JsonNull,
    ...
    async updateStagingDetail(stagingDetailId, dto, actingUserId) {
        ...
        const staging = {
            ...
            metastasis_sites: dto.metastasis_sites !== undefined
                ...
                : existing.metastasis_sites
        };
        const ihc = { ...mapIhcRowToInput(existing.ihc_results), ...(dto.ihc ?? {}) };
        const molecular = { ...mapMolecularRowToInput(existing.molecular_results), ...(dto.molecular ?? {}) };
        const validation = (0, chemo_validation_1.validateOncologyRecord)(staging, ihc, molecular);
        if (validation.hardErrors.length > 0) {
            throw new OncologyValidationError(validation.hardErrors);
        }
        ...
        await prisma_1.default.$transaction(async (tx) => {
            ...
            if (Object.keys(auditChanges).length > 0) {
                await (0, audit_service_1.logAudit)(tx, {
                    ...
                    change_summary: (0, audit_service_1.diffFields)({ ...existing, ...mapIhcRowToInput(existing.ihc_results), ...mapMolecularRowToInput(existing.molecular_results) }, auditChanges)
                });
            }
        });
        return {
    ...
    async upsertBiomarkerSubResource(stagingDetailId, kind, dto, actingUserId) {
        const existing = await this.repository.findStagingDetailById(stagingDetailId);
        if (!existing) {
            throw new Error("Staging detail not found");
        }
        const staging = {
            ...
            metastasis_sites: existing.metastasis_sites
        };
        const ihc = kind === "ihc"
            ? { ...mapIhcRowToInput(existing.ihc_results), ...dto }
            : mapIhcRowToInput(existing.ihc_results);
        const molecular = kind === "molecular"
            ? { ...mapMolecularRowToInput(existing.molecular_results), ...dto }
        ...
        const derived = (0, chemo_derivation_1.deriveOncologyFields)(staging, ihc, molecular, { patientAgeYears, icd10FromSubtype: existing.icd10_code, icdO3FromSubtype: icdO3Combined }, params);
        const existingSubResource = kind === "ihc" ? existing.ihc_results : existing.molecular_results;
        const wasCreate = !existingSubResource;
        await prisma_1.default.$transaction(async (tx) => {
            if (kind === "ihc") {
                await this.repository.upsertIhcResults(tx, stagingDetailId, dto);
            }
            else {
                await this.repository.upsertMolecularResults(tx, stagingDetailId, dto);
            }
            await this.repository.upsertDerivedFields(tx, stagingDetailId, derivedPersistPayload(derived));
            await (0, audit_service_1.logAudit)(tx, {
                entity_type: kind === "ihc" ? "ihc_results" : "molecular_results",
                entity_id: stagingDetailId,
                action: wasCreate ? audit_types_1.AUDIT_ACTION.CREATE : audit_types_1.AUDIT_ACTION.UPDATE,
                performed_by: actingUserId,
                patient_id: existing.patient_id,
                branch_id: existing.branch_id,
                change_summary: wasCreate
                    ? (0, audit_service_1.summarizeCreate)(dto)
                    : (0, audit_service_1.diffFields)(kind === "ihc" ? mapIhcRowToInput(existingSubResource) : mapMolecularRowToInput(existingSubResource), dto)
            });
        });
        return {
            staging_detail_id: stagingDetailId,
    ...
    async upsertIhc(stagingDetailId, dto, actingUserId) {
        return this.upsertBiomarkerSubResource(stagingDetailId, "ihc", dto, actingUserId);
    }
    async upsertMolecular(stagingDetailId, dto, actingUserId) {
    ...
    // thresholds.
    // ---------------------------------------------------------------
    async getStagingDetail(stagingDetailId) {
    ...
    async getDerivedFields(stagingDetailId) {
        ...
        const params = await this.repository.loadClinicalParameters();
        const her2Positive = (0, chemo_derivation_1.deriveHer2Positive)(mapIhcRowToInput(staging.ihc_results), params);
        return { ...staging.derived_fields, her2_positive: her2Positive };
    }
    attachHer2Positive(row, params) {
        const her2Positive = row.ihc_results
            ? (0, chemo_derivation_1.deriveHer2Positive)(mapIhcRowToInput(row.ihc_results), params)
            : null;
        return { ...row, her2_positive: her2Positive };
    }
}
exports.OncologyService = OncologyService;
