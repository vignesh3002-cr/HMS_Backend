...
exports.isLost = void 0;
exports.validateOncologyRecord = validateOncologyRecord;
const oncology_constants_1 = require("./oncology.constants");
Object.defineProperty(exports, "isLost", { enumerable: true, get: function () { return oncology_constants_1.isLost; } });
...
function validateOncologyRecord(staging, ihc, molecular) {
    const hardErrors = [];
    const warnings = [];
    // V-01: HER2 IHC 2+ (Equivocal) requires a resolved FISH result before save.
    if ((0, oncology_constants_1.her2IhcScore)(ihc.her2_ihc) === "2+" && (0, oncology_constants_1.isNotDoneOrPending)(ihc.her2_fish)) {
        hardErrors.push({
            rule: oncology_constants_1.VALIDATION_RULE.V01_HER2_FISH_REQUIRED,
            field: "her2_fish",
            message: "HER2 Equivocal (2+) - FISH result is mandatory before saving."
        });
    }
    // V-02: ER Positive requires ER%.
    if ((0, oncology_constants_1.isPositiveLike)(ihc.er_status) && (ihc.er_percent === null || ihc.er_percent === undefined)) {
        hardErrors.push({
            rule: oncology_constants_1.VALIDATION_RULE.V02_ER_PERCENT_REQUIRED,
            field: "er_percent",
            message: "ER% required when ER status is Positive (enter 1-100)."
        });
    }
    // V-03: PR Positive requires PR%.
    if ((0, oncology_constants_1.isPositiveLike)(ihc.pr_status) && (ihc.pr_percent === null || ihc.pr_percent === undefined)) {
        hardErrors.push({
            rule: oncology_constants_1.VALIDATION_RULE.V03_PR_PERCENT_REQUIRED,
            field: "pr_percent",
    ...
    if (ihc.pdl1_clone && !(0, oncology_constants_1.isNotDoneOrPending)(ihc.pdl1_clone)) {
        const hasScore = (ihc.pdl1_tps !== null && ihc.pdl1_tps !== undefined)
            || (ihc.pdl1_cps !== null && ihc.pdl1_cps !== undefined);
        if (!hasScore) {
            hardErrors.push({
                rule: oncology_constants_1.VALIDATION_RULE.V04_PDL1_SCORE_REQUIRED,
                field: "pdl1_tps",
    ...
    if (!staging.clinical_stage) {
        hardErrors.push({
            rule: oncology_constants_1.VALIDATION_RULE.V05_CLINICAL_STAGE_REQUIRED,
    ...
    if ((staging.m_stage ?? "").trim().toUpperCase().startsWith("M1")) {
        if (!staging.metastasis_sites || staging.metastasis_sites.length === 0) {
            hardErrors.push({
                rule: oncology_constants_1.VALIDATION_RULE.V06_METASTASIS_SITES_REQUIRED,
    ...
    if (isDmmr) {
        const missing = ["mmr_mlh1", "mmr_msh2", "mmr_msh6", "mmr_pms2"]
            .filter((field) => !ihc[field]);
        if (missing.length > 0) {
            hardErrors.push({
                rule: oncology_constants_1.VALIDATION_RULE.V07_MMR_PANEL_REQUIRED,
                field: missing.join(", "),
    ...
    if ((0, oncology_constants_1.isPositiveLike)(molecular.egfr_status) && !molecular.egfr_mutation_type) {
        warnings.push({
            rule: oncology_constants_1.VALIDATION_RULE.V08_EGFR_MUTATION_TYPE_SOFT,
