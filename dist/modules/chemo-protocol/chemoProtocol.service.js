...
exports.ChemoProtocolService = void 0;
const prisma_1 = __importDefault(require("../../config/prisma"));
const idGenerator_1 = require("../../utils/idGenerator");
const chemoProtocol_repository_1 = require("./chemoProtocol.repository");
const repository = new chemoProtocol_repository_1.ChemoProtocolRepository();
class ChemoProtocolService {
    async createProtocol(data, createdBy) {
        ...
        return prisma_1.default.$transaction(async (tx) => {
            const protocolId = await (0, idGenerator_1.generateId)(tx, "CHEMO_PROTOCOL");
    ...
    async updateProtocol(protocolId, data, updatedBy) {
    ...
    async deleteProtocol(protocolId, updatedBy) {
        const existing = await repository.findById(protocolId);
        if (!existing) {
            throw new Error("Chemotherapy protocol not found");
        }
    ...
    async restoreProtocol(protocolId, updatedBy) {
        const existing = await repository.findById(protocolId);
        if (!existing) {
            throw new Error("Chemotherapy protocol not found");
        }
    ...
    async addDrugToProtocol(protocolId, data, createdBy) {
        ...
        const existingPair = await repository.findProtocolDrugByPair(protocolId, data.drug_id);
        if (existingPair) {
            throw new Error("This drug is already part of the protocol");
        }
        return prisma_1.default.$transaction(async (tx) => {
            const protocolDrugId = await (0, idGenerator_1.generateId)(tx, "PROTOCOL_DRUG");
    ...
    async removeDrugFromProtocol(protocolDrugId) {
        const existing = await repository.findProtocolDrugById(protocolDrugId);
        if (!existing) {
            throw new Error("Protocol drug entry not found");
        }
...
exports.ChemoProtocolService = ChemoProtocolService;
