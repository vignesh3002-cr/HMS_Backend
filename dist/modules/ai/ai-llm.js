...
function isLLMConfigured() {
    ...
    if (LLM_PROVIDER === "opencode")
        return Boolean(OPENCODE_BASE_URL);
 ...
 */
async function chatWithLLM(message, systemPrompt, user, conversationMessages) {
    const tools = (0, ai_tools_1.getOpenAITools)();
    const allowedToolNames = new Set((0, ai_permissions_1.getAllowedTools)(user));
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        for (const toolCall of llmResponse.toolCalls) {
            if (!(0, ai_permissions_1.canUseTool)(user, toolCall.name)) {
            ...
            try {
                const result = await (0, ai_tool_executor_1.executeAITool)(toolCall.name, toolCall.parsedArgs, user);
...
async function callLLM(messages, tools) {
    if (LLM_PROVIDER === "opencode")
        return callOpenCode(messages, tools);
    if (LLM_PROVIDER === "nvidia")
        return callNvidia(messages, tools);
    if (LLM_PROVIDER === "google")
        return callGoogle(messages, tools);
...
async function callOpenRouter(messages, tools) {
    ...
    if (!choice)
        return { text: "No response from LLM.", toolCalls: null };
...
async function callOpenCode(messages, tools) {
    ...
    for (const m of messages) {
        if (m.role === "system")
            continue;
        if (m.role === "user")
            contextParts.push(`User: ${m.content}`);
        if (m.role === "assistant" && m.content)
            contextParts.push(`Assistant: ${m.content}`);
        if (m.role === "tool")
            contextParts.push(`Tool result: ${m.content}`);
    }
