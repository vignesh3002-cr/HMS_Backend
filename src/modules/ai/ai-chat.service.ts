import type { AIUserContext, AIChatRequest, AIChatResponse, AIConversationMessage, AIPerformedAction } from "./ai-types";
import { canUseTool } from "./ai-permissions";
import { executeAITool } from "./ai-tool-executor";
import { isLLMConfigured, chatWithLLM, buildSystemPrompt } from "./ai-llm";

interface ConversationStore {
    messages: AIConversationMessage[];
    pendingConfirmations: Map<string, { toolName: string; args: any; expiresAt: number }>;
    lastActivity: number;
}
...
function getConversation(conversationId: string | undefined): { id: string; store: ConversationStore } {
...
export async function processAIChat(
    ...
    const performedActions: AIPerformedAction[] = [];
    ...
    if (confirmMatch === "yes" || confirmMatch === "confirm" || confirmMatch === "proceed" || confirmMatch === "do it") {
        for (const [key, pending] of store.pendingConfirmations.entries()) {
            if (pending.expiresAt > Date.now()) {
                ...
                if (!canUseTool(user, pending.toolName)) {
                ...
                const result = await executeAITool(pending.toolName, pending.args, user);
    ...
    try {
        ...
        const { response: llmResponse, toolCalls } = await chatWithLLM(
            request.message,
            systemPrompt,
            user,
            recentMessages
