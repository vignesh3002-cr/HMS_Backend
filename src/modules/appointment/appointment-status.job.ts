...
import { APPOINTMENT_STATUS } from "./appointment.constants";

const DEFAULT_CONSULTATION_MINUTES = 20;
export const NOT_CHECKED_IN_JOB_INTERVAL_MS = 5 * 60 * 1000;
...
export async function transitionElapsedAppointments(): Promise<number> {

    const now = new Date();
    const todayUtcMidnight = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()));

    const candidates = await prisma.appointment_history.findMany({
        where: {
            status: {
                in: [APPOINTMENT_STATUS.SCHEDULED, APPOINTMENT_STATUS.RESCHEDULED]
            },
    ...
    const result = await prisma.appointment_history.updateMany({
        ...
        data: { status: APPOINTMENT_STATUS.NOT_CHECKED_IN }
    });
...
export function startAppointmentStatusJob(): void {
    ...
    }, NOT_CHECKED_IN_JOB_INTERVAL_MS);
...
export function stopAppointmentStatusJob(): void {
