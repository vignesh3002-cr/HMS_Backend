...
type AuthUser = NonNullable<
  Awaited<ReturnType<AuthRepository["findUserByUsername"]>>
>;
...
export class AuthService {
  ...
  private assertAccountUsable(user: AuthUser) {
    ...
    if (!isTopLevelAdmin) {
      ...
      const mappings = (user.user_branch_mapping as any[]) || [];
      ...
      if (activeMappings.length === 0) {
        throw new Error(
          "No branch has been assigned to your account. Please contact the Head Admin."
        );
      }
    }
  }
  ...
  async sendOtp(username: string) {
    ...
    return {
      message: `OTP sent to ${email.replace(
        /^(.{2}).*(@.*)$/,
        "$1***$2"
      )}`,
    };
