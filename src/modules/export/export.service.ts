...
const isPhotoColumn = (key: string): boolean => /photo|picture|avatar/i.test(key);
...
function toCsvRows(
    ...
    relationKeys: string[],
    readableColumns: Array<{ key: string; value: (row: any) => string }>
): { rows: Array<Record<string, unknown>>; columns: string[] } {
    const flat = rows.map((row) => {
        const rest: Record<string, unknown> = { ...row };
        for (const key of relationKeys) delete rest[key];
        for (const col of readableColumns) rest[col.key] = col.value(row);
        return rest;
    });

    const allKeys = flat[0] ? Object.keys(flat[0]) : [];
    const columns = allKeys.filter(
        (key) => !EXCLUDED_COLUMNS.has(key) && !isPhotoColumn(key)
    );
    return { rows: flat, columns };
}

const formatBranchName = (branch?: { branch_name: string; branch_area: string | null } | null): string =>
    branch ? (branch.branch_area ? `${branch.branch_name} (${branch.branch_area})` : branch.branch_name) : "";
...
export class ExportService {
    ...
    async buildEmployeesCsv(query: {
        ...
        const userTableWhere: Record<string, unknown> = {};
        if (query.roleType) userTableWhere.role_type = query.roleType;
        if (query.excludeRoleType) userTableWhere.role_type = { not: query.excludeRoleType };
    ...
    async buildPatientsCsv(query: { branchId?: string }): Promise<string> {
    ...
    async buildAppointmentsCsv(query: {
        ...
        const { rows, columns } = toCsvRows(
            appointments,
            ["branch", "department_master", "patient_bio_data"],
            [
                {
                    key: "patient_name",
                    value: (a) =>
