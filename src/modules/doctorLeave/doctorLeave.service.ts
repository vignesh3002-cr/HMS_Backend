...
import { DoctorLeaveRepository } from "./doctorLeave.repository";
import {
    ApplyDoctorLeaveDto,
    ApproveDoctorLeaveDto,
    RejectDoctorLeaveDto,
    GetDoctorLeaveQuery,
    QueueRescheduleDto
} from "./doctorLeave.types";
import { LEAVE_STATUS } from "./doctorLeave.constants";
import { RESCHEDULE_QUEUE_STATUS } from "../doctor-transfer/doctorTransfer.constants";
import { generateId } from "../../utils/idGenerator";

export class DoctorLeaveService {

    private repository = new DoctorLeaveRepository();


    private async resolveDoctor(employeeId: string) {

        const employee =
            await this.repository.findDoctorWithRole(employeeId);
    ...
    async applyLeave(
        ...
        if (dto.replacement_employee_id) {
            ...
            const replacement =
                await this.repository.findReplacementDoctor(
            ...
            if (replacement.user_table?.role_type !== "DOCTOR") {
                throw new Error(
                    "Replacement employee is not a doctor"
                );
            }

            if (replacement.emp_status === false) {
                throw new Error(
                    "Replacement doctor is inactive"
                );
            }

            if (dto.replacement_employee_id === employeeId) {
                throw new Error(
                    "Replacement doctor cannot be the same doctor"
                );
            }
        }
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const leaveId =
                await this.repository.generateLeaveId(tx);


            const leave =
                await this.repository.applyLeave(tx, {
                    ...
                    replacement_employee_id:
                        dto.replacement_employee_id ?? null,
                    ...
                    branch_id: doctor.branch_id as string
    ...
    async approveLeave(
        leaveId: string,
        dto: ApproveDoctorLeaveDto,
        approvedBy: string
    ) {

        const leave =
            await this.repository.findLeaveById(leaveId);
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const updatedLeave =
                await this.repository.approveLeave(
                    tx,
                    leaveId,
                    {
    ...
    async rejectLeave(
        ...
        if (!dto.remarks?.trim()) {
            throw new Error(
                "Remarks are required while rejecting a leave request"
            );
        }
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const updatedLeave =
                await this.repository.rejectLeave(
                    tx,
                    leaveId,
                    {
    ...
    async getLeaveConflicts(
        employeeId: string,
        dto: QueueRescheduleDto
    ) {

        const startDate = new Date(
            `${dto.date_from}T00:00:00.000Z`
        );
        const endDate = new Date(
            `${dto.date_to}T00:00:00.000Z`
        );
        ...
        const appointments =
            await this.repository.findActiveAppointmentsInRange(
                prisma,
                employeeId,
                dto.date_from,
                dto.date_to
    ...
    async queueRescheduleForLeave(
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const appointment of appointments) {
                ...
                await this.repository.markAppointmentRescheduleRequired(
                    tx,
                    appointment.appointment_id
                );
                ...
                await this.repository.createRescheduleQueueEntry(
                    tx,
                    {
                        queue_id: queueId,
                        appointment_id:
                        ...
                        reason:
                            `DOCTOR_LEAVE${dto.reason ? `: ${dto.reason}` : ""}`,
