...
import { CreateBranchDto, UpdateBranchDto, NewBranchAdminDto, AssignableAdminDto } from "./branch.types";
...
export class BranchService {
  ...
  async getBranchById(branchId: string) {
    ...
    const { user_branch_mapping, ...branchFields } = branch as any;
  ...
  async createBranch(
    ...
    if (data.branch_code) {
      ...
      if (existingBranchCode) throw new Error("Branch code already exists");
    }
    ...
    if (data.admin_mode === "NEW") {
      ...
      if (!user) throw new Error("Selected admin not found");
      if (user.role_type !== "BRANCH_ADMIN") throw new Error("Selected user must have BRANCH_ADMIN role");
    }

    // Hash password for NEW mode
    const hashedPassword = data.admin?.password
      ? await bcrypt.hash(data.admin.password, Number(process.env.BCRYPT_SALT_ROUNDS))
      : undefined;

    return await prisma.$transaction(async (tx) => {
      // 1. Create branch FIRST (FK target for user_table.branch_id, employees.branch_id)
      const branchId = await generateId(tx, "BRANCH");
      ...
      if (data.admin_mode === "NEW") {
        ...
        const employeeId = await generateId(tx, "BRANCH_ADMIN");
        adminEmployee = await tx.employees.create({
          data: {
            ...
            emp_gender: data.admin!.gender,
            emp_DOB: data.admin!.dob,
  ...
  async getAssignableAdmins(search?: string): Promise<AssignableAdminDto[]> {
    const where = search
      ? {
          OR: [
            { username: { contains: search, mode: "insensitive" as const } },
            { employees: { first_name: { contains: search, mode: "insensitive" as const } } },
            { employees: { email: { contains: search, mode: "insensitive" as const } } },
          ],
          role_type: "BRANCH_ADMIN",
        }
      : { role_type: "BRANCH_ADMIN" };
  ...
  async assignAdmin(branchId: string, userId: string) {
    const branch = await repository.getBranchById(branchId);
    if (!branch) throw new Error("Branch not found");
    ...
    if (user.role_type !== "BRANCH_ADMIN") throw new Error("User must have BRANCH_ADMIN role");
  ...
  async unassignAdmin(userId: string) {
    const user = await prisma.user_table.findUnique({ where: { user_id: userId } });
    if (!user) throw new Error("User not found");
    if (user.role_type !== "BRANCH_ADMIN") throw new Error("User must have BRANCH_ADMIN role");
