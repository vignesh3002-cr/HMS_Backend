...
import { TreatmentPlanRepository } from "./treatmentPlan.repository";
import {
 ...
 */
function calculateBsa(heightCm?: number, weightKg?: number): number | undefined {
...
export class TreatmentPlanService {
    ...
    private async validateReferences(data: {
        ...
        if (data.treatment_intent_id) {
            ...
            const intent = await repository.findTreatmentIntent(
                data.treatment_intent_id
            );
    ...
    async createTreatmentPlan(data: CreateTreatmentPlanDto, createdBy: string) {
        ...
        if (diagnosis.patient_id !== data.patient_id) {
            throw new Error(
                "The selected diagnosis does not belong to the given patient"
            );
        }
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const treatmentPlanId = await generateId(tx, "TREATMENT_PLAN");
    ...
    async getTreatmentPlans(query: GetTreatmentPlansQuery) {

        return repository.list(query);

    }

    async getTreatmentPlanById(treatmentPlanId: string) {
     ...
     */
    async approveTreatmentPlan(treatmentPlanId: string, approvedBy: string) {
        ...
        if (existing.plan_status === "Cancelled" || existing.plan_status === "Completed") {
            throw new Error(
                `Cannot approve a plan that is already ${existing.plan_status}`
            );
        }
    ...
    async deleteTreatmentPlan(treatmentPlanId: string, updatedBy: string) {
    ...
    async restoreTreatmentPlan(treatmentPlanId: string, updatedBy: string) {
